...
#define ECS_CRASH_IMPLEMENTATION(error_string, crash_function, ...) ECS_STACK_CAPACITY_STREAM(char, base_characters, 512); base_characters.Copy(ToStream("Engine crash in file {#}, function {#}, line {#}. ")); \
	base_characters.AddStreamSafe(ToStream(error_string)); \
...
#define ECS_CRASH_IMPLEMENTATION_EX(error_string, crash_function, file, function, line, ...) ECS_STACK_CAPACITY_STREAM(char, base_characters, 512); base_characters.Copy(ToStream("Engine crash in file {#}, function {#}, line {#}. ")); \
	base_characters.AddStreamSafe(ToStream(error_string)); \
	base_characters[base_characters.size] = '\0'; \
