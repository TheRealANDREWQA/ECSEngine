	...
	struct StableReferenceStream {
		...
		void AllocateIndex(unsigned int index) {
			...
			if constexpr (queue_indirection_list) {
				...
				if (indirection_list_start_index == 0) {
					// AAAA BBBBBB
					indirection_index = SearchBytes(indirection_list + size, capacity - size, index);
					...
					if (indirection_list_start_index + size < capacity) {
						...
						indirection_index = SearchBytes(indirection_list, indirection_list_start_index, index);
						if (indirection_index == -1) {
							unsigned int offset = indirection_list_start_index + size;
							indirection_index = SearchBytes(indirection_list + offset, capacity - offset, index);
						...
						if (indirection_list_start_index + size == capacity) {
							// BBBBBB AAAAA
							// One search
							indirection_index = SearchBytes(indirection_list, capacity - size, index);
						}
						else {
							// AA BBBBBB AAA
							// 1 search in the middle of the wrap around
							unsigned int offset = indirection_list_start_index + size - capacity;
							indirection_index = SearchBytes(indirection_list + offset, capacity - size, index);
							indirection_index = indirection_index == -1 ? -1 : indirection_index + offset;
						}
					}
				}
		...
		void Remove(unsigned int index) {
			if constexpr (!queue_indirection_list) {
				...
				size_t free_list_index = SearchBytes(indirection_list, size, index);
				...
				if (indirection_list_start_index + size < capacity) {
					// No wrapping
					free_list_index = SearchBytes(indirection_list + indirection_list_start_index, size, index);
					ECS_ASSERT(free_list_index != -1);
					...
					free_list_index = SearchBytes(indirection_list + indirection_list_start_index, first_chunk_count, index);
					if (free_list_index == -1) {
						free_list_index = SearchBytes(indirection_list, size - first_chunk_count, index);
						ECS_ASSERT(free_list_index != -1);
					}
					else {
						free_list_index += indirection_list_start_index;
		...
		void Initialize(AllocatorPolymorphic allocator, unsigned int _capacity, DebugInfo debug_info = ECS_DEBUG_INFO) {
			...
			if (_capacity > 0) {
				size_t memory_size = MemoryOf(_capacity);
				allocation = Allocate(allocator, memory_size, alignof(void*), debug_info);
			}
			InitializeFromBuffer(allocation, _capacity);
		}
