...
#undef LOOP_ITERATION
	...
	ComponentSignature VectorComponentSignature::ToNormalSignature(Component* components) const
	{
		unsigned char count = Count();
		value.store_partial(count, components);

		// The components are incremented in order to distinguish from the termination condition
