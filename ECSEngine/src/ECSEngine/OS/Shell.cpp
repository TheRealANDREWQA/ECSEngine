...
namespace ECSEngine {
	...
	namespace OS {
		...
		bool ShellRunCommand(Stream<wchar_t> command, Stream<wchar_t> start_directory) {
			NULL_TERMINATE_WIDE(start_directory);
			...
			HINSTANCE value = ShellExecute(NULL, L"runas", L"C:\\Windows\\System32\\cmd.exe", full_command.buffer, start_directory.buffer, SW_HIDE);
