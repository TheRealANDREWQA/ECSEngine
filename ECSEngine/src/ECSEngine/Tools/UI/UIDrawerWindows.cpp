...
#pragma region Messages
			...
			auto draw_sentence = [&](unsigned int console_message_index, float& vertical_span) -> bool {
				...
				if (region_width_changed || vertical_span == 0.0f || drawer.ValidatePositionY(0, drawer.GetCurrentPosition(), { 0.0f, vertical_span })) {
					...
					// since that was accounted for by the next row done earlier, and go to the next row
					drawer.FinalizeRectangle(0, drawer.GetCurrentPosition(), { 0.0f, vertical_span - drawer.GetLayoutDescriptor()->next_row_y_offset });
					return false;
				}
			};
