...
bool CreateProjectMetafile(EditorState* editor_state, ECSEngine::containers::CapacityStream<wchar_t> path);

void CreateProjectMissingMetafiles(EditorState* editor_state);

// It changes the Metafile folder to Assets and removes the .meta
void FromMetafilePathToAssetsPath(const EditorState* editor_state, ECSEngine::containers::CapacityStream<wchar_t>& demangled_path, ECSEngine::containers::Stream<wchar_t> path);

// It changes the Assets folder to Metafile and add .meta
void FromAssetsPathToMetafilePath(const EditorState* editor_state, ECSEngine::containers::CapacityStream<wchar_t>& mangled_path, ECSEngine::containers::Stream<wchar_t> path);

// Writes into the file the default values for that metafile
void WriteMetafileDefaultValues(ECSEngine::ECS_FILE_HANDLE file_handle, ECSEngine::containers::Stream<wchar_t> path);