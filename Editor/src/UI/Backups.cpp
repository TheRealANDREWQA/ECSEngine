...
void DeleteBackupAction(ActionData* action_data) {
	...
	absolute_path.AddStreamSafe(ToStream(data->path));
	absolute_path[absolute_path.size] = L'\0';

	bool success = RemoveFolder(absolute_path);
	if (!success) {
		EditorSetConsoleError(ToStream("Could not delete backup."));
	}
}
...
void RestoreBackupAction(ActionData* action_data) {
	...
	if (success) {
		EditorSetConsoleInfo(ToStream("The backup was restored successfully."));
	}
}
...
void BackupsDraw(void* window_data, void* drawer_descriptor, bool initialize) {
	...
	auto recover_last_backup = [](ActionData* action_data) {
		...
		ForEachDirectory(backup_folder, &data, [](const wchar_t* path, void* _data) {
			...
			if (function::IsDateLater(data->current_date, date)) {
				...
				data->folder->AddStreamSafe(function::PathFilename(ToStream(path)));
			}
		...
		if (data.base_folder_size == backup_folder.size) {
			EditorSetConsoleError(ToStream("No backup was found when trying to restore latest backup."));
			return;
		}

		bool success = LoadProjectBackup(editor_state, backup_folder);
		if (success) {
			EditorSetConsoleInfo(ToStream("Latest backup was restored with success."));
		}
	};
	...
	ForEachDirectory(backup_folder, &functor_data, [](const wchar_t* path, void* _data) {
		FunctorData* data = (FunctorData*)_data;
		if (data->backup_total_byte_size == 0) {
			data->drawer->Text(UI_CONFIG_DO_NOT_CACHE, *data->config, "Backups at: ");
			data->drawer->NextRow();
		}
		...
		data->drawer->Text(UI_CONFIG_DO_NOT_CACHE, *data->config, ascii_filename.buffer);
		...
		data->drawer->Text(UI_CONFIG_DO_NOT_CACHE, *data->config, byte_size_string.buffer);
	...
	if (functor_data.backup_total_byte_size == 0) {
		drawer.Text(UI_CONFIG_DO_NOT_CACHE, config, "There are no backups.");
		drawer.NextRow();
	}
	else {
		ECS_STACK_CAPACITY_STREAM(char, total_backup_size, 128);
		total_backup_size.Copy(ToStream("Total backup byte size: "));
		function::ConvertByteSizeToString(functor_data.backup_total_byte_size, total_backup_size);
		drawer.Text(UI_CONFIG_DO_NOT_CACHE, config, total_backup_size.buffer);
		drawer.NextRow();
	}
	...
	drawer.Button(UI_CONFIG_DO_NOT_CACHE | UI_CONFIG_WINDOW_DEPENDENT_SIZE | UI_CONFIG_ACTIVE_STATE, config, "Restore last backup", { recover_last_backup, editor_state, 0 });
	config.flag_count--;
	drawer.NextRow();
	...
	auto manual_backup = [](ActionData* action_data) {
		...
		if (success) {
			...
			EditorSetConsoleInfo(ToStream("Manual backup saved successfully."));
			AddProjectNeedsBackupTime(editor_state, 60);
		}
		else {
			EditorSetConsoleError(ToStream("Could not save manual project backup."));
		}
	};
	...
	config.AddFlag(active_state);
	drawer.Button(UI_CONFIG_DO_NOT_CACHE | UI_CONFIG_WINDOW_DEPENDENT_SIZE | UI_CONFIG_ACTIVE_STATE, config, "Manual backup", { manual_backup, editor_state, 0 });
}
