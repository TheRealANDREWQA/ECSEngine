...
ComponentFunctions EditorComponents::GetAutoGeneratedComponentFunctions(Stream<char> component_name, CapacityStream<void>* stack_memory) const
{
	return GetReflectionTypeRuntimeComponentFunctions(internal_manager, GetType(component_name), stack_memory);
}

// ----------------------------------------------------------------------------------------------

SharedComponentCompareEntry EditorComponents::GetAutoGeneratedCompareEntry(Stream<char> component_name, CapacityStream<void>* stack_memory) const
{
	return GetReflectionTypeRuntimeCompareEntry(internal_manager, GetType(component_name), stack_memory);
}
	...
	auto functor = [](const ReflectionType* type, void* _data) {
		...
		if (!data->entity_manager->ExistsComponent(component_id)) {
			...
			if (module_component_functions != nullptr && module_component_functions->copy_function != nullptr && module_component_functions->deallocate_function != nullptr) {
				...
				component_functions = GetReflectionTypeRuntimeComponentFunctions(data->reflection_manager, type, &stack_memory);
			}
	...
	auto shared_functor = [](const ReflectionType* type, void* _data) {
		...
		if (!data->entity_manager->ExistsSharedComponent(component_id)) {
			...
			if (module_component_functions != nullptr && module_component_functions->compare_function != nullptr) {
				...
				compare_entry = GetReflectionTypeRuntimeCompareEntry(data->reflection_manager, type, &compare_stack_memory);
			}
